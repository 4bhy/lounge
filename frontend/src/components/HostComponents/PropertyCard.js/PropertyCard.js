...
const PropertyCard = () => {
    ...
    console.log(listHostPropertyData, "00000000");
    const dispatch = useDispatch()

    useEffect(() => {
        dispatch(listHostProperties())
    }, [])

    const showMore = () => {
        setValue( listHostPropertyData.hostProperties.length)
        setShow(true)
    }
    ...
    return (
        <div>
            <div class="flex flex-wrap justify-end w-full mb-4 p-4">
                <div class="w-full mb-6 lg:mb-0">
                    <h1 class="sm:text-4xl text-5xl font-medium font-bold title-font mb-2 text-gray-900">My Properties</h1>
                    <div class="h-1 w-20 bg-indigo-500 rounded"></div>
                </div>
                <div>
                    <span className='sm:text-2xl text-2xl font-medium font-bold justify-end title-font mb-2 text-gray-900'><Link to="/host/add-property">Add New+</Link></span>
                                    ...
                                    <div class="flex justify-center relative rounded-lg overflow-hidden h-52">
                                        <div className="transition-transform duration-500 transform ease-in-out hover:scale-110 w-full" style={{ position: 'relative', height: '100%', width: '100%' }}>
                                            <div className="absolute inset-0 bg-black opacity-100">
                                                <img src={data.pic[0]} style={{ height: '100%', width: '100%', objectFit: 'cover' }} />
                                            </div>
                                        </div>
